...
export function Doctors() {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      if (!userId) return
      const { data } = await supabase.from('users_favorites').select('*').eq('user_id', userId)
      setFavorites(data || [])
    }
    fetchFavorites()
  }, [userId])
  ...
  const toggleFavorite = async (doctorId: string) => {
    ...
    if (favorite) {
      ...
      await supabase.from('users_favorites').insert({ user_id: userId, doctor_id: doctorId })
    }
    const { data } = await supabase.from('users_favorites').select('*').eq('user_id', userId)
    setFavorites(data || [])
  }
  ...
  const handleBooking = (doctor: Doctor) => {
    router.push(`/booking?doctor_id=${doctor.id}&doctor_name=${encodeURIComponent(doctor.name)}`)
  }
    ...
    <section className="py-20 bg-gradient-to-b from-white via-gray-50 to-white relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 text-lg">Kết nối với những chuyên gia y tế hàng đầu</p>
        </div>
